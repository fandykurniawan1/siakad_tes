...
@section('content')
    <ul class="blocks-xs-100 blocks-lg-2">
        ...
        <li class="block-70 no-mrg-btm">
            <form class="form-horizontal" id="form" action="{{ route('backend.management.role.update', $role->id) }}" method="POST">
                {!! csrf_field() !!}
                {!! method_field('PUT') !!}
                                ...
                                @foreach ($permissionGroups->chunk(ceil(count($permissionGroups) / 2))[0] as $groupName => $permissions)
                                    <h4 class="example-title">
                                        ...
                                        <div class="pull-right" style="padding-right: 8px;">
                                            Select All
                                            <input type="checkbox" class="icheckbox-yellow selectAll" data-group="{{ $groupName }}" data-plugin="iCheck" data-checkbox-class="icheckbox_flat-yellow">
                                        </div>
                                    </h4>
                                    <table class="table">
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="checkbox" class="icheckbox-yellow" data-group="{{ $groupName }}" value="{{ $permission->id }}" name="permission_ids[]" data-plugin="iCheck" data-checkbox-class="icheckbox_flat-yellow" {{ in_array($permission->id, $rolePermissionIds) ? 'checked' : '' }}></td>
                                        </tr>
                                        @endforeach
                                    </table>
                                @endforeach
