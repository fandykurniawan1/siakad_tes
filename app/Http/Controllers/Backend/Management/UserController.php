...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            'name' => 'required',
            'email' => 'required|email|unique:users,email,NULL,id,deleted_at,NULL,platform,Admin',
            'username' => 'required|unique:users,username,NULL,id,deleted_at,NULL,platform,Admin',
        ...
        return redirect()->route('backend.management.user.index')->with('notif_success', 'New user has been saved successfully!');
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $user = User::withoutGlobalScopes(['active', 'platform'])->findOrFail($id);
        $roles = Role::orderBy('display_name', 'asc')->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->validate($request, [
            'name' => 'required',
            'email' => 'required|email|unique:users,email,' . $user->id . ',id,deleted_at,NULL,platform,Admin',
            'username' => 'required|unique:users,username,' . $user->id . ',id,deleted_at,NULL,platform,Admin',
            'role' => 'required|uuid|exists:roles,id',
            'active' => 'nullable'
        ]);
        ...
        return redirect()->route('backend.management.user.index')->with('notif_success', 'User has been updated successfully!');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('backend.management.user.index')->with('notif_success', 'User has been deleted successfully!');
    }
    ...
    public function getData(Request $request)
    {
        ...
        return Datatables::of($users)
                    ...
                    ->addColumn('action', function($user) {
                        $edit = '<a href="' . route('backend.management.user.edit', $user->id) . '" class="btn btn-sm btn-icon text-primary tl-tip" data-toggle="tooltip" data-original-title="Edit user"><i class="icon wb-edit" aria-hidden="true"></i></a>';
                        $changePassword = '<a href="' . route('backend.management.user.other.change-password', $user->id) . '" class="btn btn-sm btn-icon text-info tl-tip"
                                data-toggle="tooltip" data-original-title="Change Password"><i class="icon fa-key" aria-hidden="true"></i></a>';
                        $delete = '<a class="btn btn-sm btn-icon text-danger tl-tip" data-href="' . route('backend.management.user.destroy', $user->id) . '" data-toggle="modal" data-target="#confirm-delete-modal" data-original-title="Delete user"><i class="icon wb-trash" aria-hidden="true"></i></a>';
